<?php

declare(strict_types=1);

namespace App\Features\Testy;

class TestyConst
{
    // Application constants
    public const VIEW_TESTY_INDEX = 'testy/index';
    public const VIEW_TESTY_PLACEHOLDER = 'testy/placeholder';
    public const VIEW_TESTY_TESTLOGGER = 'testy/testlogger';
    public const VIEW_TESTY_TESTSESSION = 'testy/testsession';
    public const VIEW_TESTY_TESTDATABASE = 'testy/testdatabase';
    public const VIEW_TESTY_CONTACT = 'testy/contact';
    public const VIEW_TESTY_CONTACTVIEWTEST = 'testy/contactviewtest';
    //public const VIEW_TESTY_EMAILTEST = 'testy/emailtester';
    public const VIEW_TESTY_EMAILTEST = 'testy/emailtest';
}
